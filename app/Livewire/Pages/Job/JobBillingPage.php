...
use App\Domains\Job\Billing\JobBillingService;
...
class JobBillingPage extends Component
{
    ...
    public function result(): array
    {
        ...
        return JobBillingService::getList($this->status, $request);
    }
    ...
    {
        return JobBillingService::getOffices();
    }
    ...
    public function getStatusLabelProperty(): string
    {
        return match ($this->status) {
            'belum' => 'Pekerjaan Belum Lengkap',
            'lengkap' => 'Pekerjaan Lengkap',
            'selesai' => 'Pekerjaan Selesai',
            default => 'Status Pekerjaan',
        };
    }
        ...
        <div class="flex h-full min-h-screen flex-col overflow-hidden">
            ...
            @else
                ...
                <div class="flex-1 overflow-auto border-y border-neutral-300 bg-white dark:border-neutral-800 dark:bg-black">
                    {{-- Loading Skeleton --}}
                    <div wire:loading>
                        <x-job.billing-skeleton :rows="20" />
                    </div>
                    ...
                    <div wire:loading.remove class="h-full">
                        <flux:table container:class="h-full w-full">
                            <flux:table.columns sticky class="bg-white/80 backdrop-blur-4xl border-b border-zinc-200/60  dark:bg-zinc-950/60 dark:border-zinc-700/60">
                                ...
                                <flux:table.column class="w-16 shrink-0">Tanggal</flux:table.column>
                                <flux:table.column class="w-32 shrink-0">Kantor</flux:table.column>
                                <flux:table.column class="w-90!">Lokasi</flux:table.column>
                                <flux:table.column class="w-28 shrink-0">Segmentasi</flux:table.column>
                                <flux:table.column class="w-40 shrink-0">Syarat Belum</flux:table.column>
                                <flux:table.column class="w-32 shrink-0 text-right pr-2!">Nilai</flux:table.column>
                            </flux:table.columns>

                            <flux:table.rows>
                                @foreach($this->data() as $row)
                                <flux:table.row
                                    :key="$row->id_kontrak_tagihan"
                                    wire:click="$dispatch('open-contract-invoice-detail', { idKontrakTagihan: '{{ $row->id_kontrak_tagihan }}' })"
                                ...
                                >
                                    ...
                                    <flux:table.cell>
                                        <div class="max-w-2xl">
                                            <div class="font-medium text-zinc-900 dark:text-white truncate" title="{{ $row->pelanggan }}">
                                                {{ $row->pelanggan }} -  <span class="text-sm text-accent truncate">{{ $row->no_kontrak }}</span>
                                            </div>
                                            <div class="text-sm text-zinc-500 dark:text-zinc-400 truncate" title="{{ $row->alamat }}">
                                                {{ $row->alamat }}
                                            </div>
                                        </div>
                                    </flux:table.cell>
                                    ...
                                    <flux:table.cell>
                                        <div class="text-sm text-zinc-600 dark:text-zinc-400 truncate" title="{{ $row->segmentasi ?? '-' }}">
                                            {{ $row->segmentasi ?? '-' }}
                                        </div>
                                    </flux:table.cell>

                                    <flux:table.cell>
                                        <div class="text-sm text-zinc-600 dark:text-zinc-400 truncate" title="{{ $row->dokumen_belum ?? '-' }}">
                                            {{ $row->dokumen_belum ?? '-' }}
                                        </div>
                                    </flux:table.cell>
